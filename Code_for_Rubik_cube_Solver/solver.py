...
class SolverThread(thr.Thread):
        ...
        # SolverThread(cc, i % 3, i // 3, max_length, timeout, s_time, solutions, terminated, [999])
    def __init__(self, cb_cube, rot, inv, ret_length, timeout, start_time, solutions, terminated, shortest_length):
        """
...
# takes a string of color, max-length of moves and time to solve
def solve(colorConfig, max_length=20, timeout=3):
    fc = face.FaceCube()
    
    # convert the colorConfig to string format
    s = fc.from_string(colorConfig)
    if s != cubie.CUBE_OK:
